...
>>>>>>> 42e18b34218841568c75a114c6b308ba795653f0
  }
...
=======
  ...
  function SalesTimeChart(transactions) {
    ...
    const salesByCategory = timeCategories.map((category) => {
      return monthValues.map((month) => {
        return transactions.filter((item) => item.periode_waktu === category && item.periode_bulanan === month).reduce((total, item) => total + parseFloat(item.unit_price), 0);
      });
    });
